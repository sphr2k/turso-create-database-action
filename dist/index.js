...
function isTursoClientError(error) {
...
async function run() {
    try {
        ...
        if (replace && !existingDatabaseName) {
            core.setFailed('The replace option can only be used when existing_database_name is provided to prevent accidental database deletion.');
            return;
        }
        ...
        if (!resolvedGroupName) {
            core.info(`Fetching group information for database: ${existingDatabaseName}`);
            try {
        ...
        if (replace) {
            ...
            try {
                ...
                try {
                    ...
                    core.info(`✓ Successfully deleted existing database fork '${newDatabaseName}'`);
                }
                catch (error) {
                    if (isTursoClientError(error)) {
                        core.setFailed(`Failed to delete existing database fork: ${error.message}`);
                    }
                    else if (error instanceof Error) {
                        core.setFailed(`Failed to delete existing database fork: ${error.message}`);
                    }
                    else {
                        core.setFailed('Failed to delete existing database fork: Unknown error');
                    }
                    return;
                }
            }
                ...
                if (isTursoClientError(error)) {
                    ...
                    if (errorMsg.includes('404') || errorMsg.includes('not found')) {
                        core.info(`Database fork '${newDatabaseName}' does not exist, skipping deletion`);
                    }
                    else {
                        // Non-404 errors indicate a real problem (auth, network, etc.)
                        // Fail rather than continue to avoid masking issues
                        core.setFailed(`Failed to check if database fork exists: ${error.message}`);
                        return;
                    }
                }
                else if (error instanceof Error) {
        ...
        core.info(`Creating database fork '${newDatabaseName}' in group '${resolvedGroupName}' from seed '${existingDatabaseName}'`);
        let createdDatabase;
        try {
        ...
        const hostname = createdDatabase.hostname ||
        ...
        if (createDatabaseToken) {
            core.info(`Creating authentication token for database: ${newDatabaseName}`);
            try {
